...
<table width="100%" border="0">
  ...
  $s=mysql_query("select * from tblpembeli where idpembeli='".$_SESSION['pembeli']."'");
  $array=(mysql_fetch_array($s));
  ?>
  <tr>
    <td colspan="3"><p>
      <judul><strong>Detail Piutang <? echo " Sdr. "; echo $array['namapembeli'];echo " - "; echo $array['hp']; ?> </strong></judul>
    </p>
  ...
  <tr>
	...
	?>
        <strong>Tanggal : <? echo $_POST['tgl'].'-'.$_POST['bln'].'-'.$_POST['thn'];?> s/d <? echo $_POST['tgl2'].'-'.$_POST['bln2'].'-'.$_POST['thn2'];?> </strong> <marquee scrolldelay="200"  direction="left" width="200" behavior="alternate"onmouseover="this.stop()" onMouseOut="this.start()"><a  class="tombol" href="cetak/lappiutang.php?thn=<? echo $_POST['thn']?>&bln=<? echo $_POST['bln']?>&tgl=<? echo $_POST['tgl']?>&thn2=<? echo $_POST['thn2']?>&bln2=<? echo $_POST['bln2']?>&tgl2=<? echo $_POST['tgl2']?>" target="_blank">--Cetak--</a></marquee>
      </div>
      <table >
        <tr>
        ...
        <?
	   //starting point pembeli
		$start=mysql_query("SELECT jumlah FROM start WHERE objek='".$_SESSION['pembeli']."'");
	  	$mulai=mysql_fetch_array($start);
		
		$sld=$mulai['jumlah'];
		
	   //mengambil nilai dari tanggal sebelumnya
	   $t=mysql_query("SELECT rekap.*, tblpembeli.namapembeli, tblpembeli.perusahaan FROM rekap LEFT JOIN tblpembeli ON rekap.kepada=tblpembeli.idpembeli WHERE tanggal<'$tgl' && kepada='".$_SESSION['pembeli']."'");
	   ...
	   $no=1; 
	   $sql=mysql_query("SELECT rekap.*, tblpembeli.namapembeli, tblpembeli.perusahaan FROM rekap LEFT JOIN tblpembeli ON rekap.kepada=tblpembeli.idpembeli WHERE tanggal>='$tgl' && tanggal<='$tgl2' && kepada='".$_SESSION['pembeli']."' ORDER BY tanggal ASC");
	   ...
	   ?>
        <tr>
          ...
          <td bgcolor="<? echo $warna;?>" class="corner10"><? echo $tampil['catatan']; if($tampil['trx']!=0){echo "Transaksi";}else {echo "Pembayaran";}?></td>
