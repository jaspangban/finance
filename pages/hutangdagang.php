...
<body onLoad="MM_preloadImages('../images/print1.png')"><judul>
  <div align="right" class="judul">HUTANG DAGANG
...
<form id="lihat" name="lihat" method="post" action="?p=hutangdagang">
  <label></label>
  <label>
...
<p><a href="cetak/lapdetailhutang.php?bln=<? echo $_POST['bln'];?>&thn=<? echo $_POST['thn'];?>" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Cetak','','../images/print1.png',1)" target="_blank"><img src="../images/print2.png" alt="Cetak" name="Cetak" width="55" height="43" border="0" id="Cetak" /></a>
<table  >
        <tr>
          <td width="5%" height="28" class="headertbl"><div align="center"><strong>No</strong></div></td>
          <td width="17%" class="headertbl"><div align="center"><strong>Penjual</strong></div></td>
          ...
          <td width="16%" class="headertbl"><div align="center"><strong><? if($_POST){}else{?>Hutang<? }?></strong></div></td>
          <td width="10%">&nbsp;</td>
        </tr>
       <?
	   ...
	   $sql=mysql_query("SELECT sum(trx) as transaksi, sum(byr) as pembayaran, sum(sisa) as sisastart, tblpenjual.* FROM rekap2 left join tblpenjual on rekap2.kepada=tblpenjual.idpenjual where status='hutang' and MONTH(tanggal)='".$_POST['bln']."' and YEAR(tanggal)='".$_POST['thn']."' group by kepada");}else{
...
//belang
	   ...
	   ?>
	    ...
	    <tr >
          ...
          <td><div align="center"><a href="?p=session&amp;hutang=<? echo $tampil['idpenjual']?>">detail</a></div></td>
	    </tr><? }?>
	    <tr >
	      <td colspan="6" ><hr /></td>
	      <td>&nbsp;</td>
	    ...
	    <? 
		if($_POST['bln']!=NULL and $_POST['thn']!=NULL){
 $sql=mysql_query("SELECT sum(trx) as gtransaksi, sum(byr) as gpembayaran, sum(sisa) as gsisastart FROM rekap2 where status='hutang'  and MONTH(tanggal)='".$_POST['bln']."' and YEAR(tanggal)='".$_POST['thn']."'");
}else{
